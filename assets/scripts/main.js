...
function initializeServiceWorker() {
  ...
  if('serviceWorker' in navigator)
  {
    window.addEventListener('load', async () => 
    {
      try
      {
        const registerWorker = await navigator.serviceWorker.register('./sw.js', {scope: '/'});
        if(registerWorker.active)
        {
      ...
      {
        console.log(`The service worker registration has failed with error ${error}.`);
      }
    })
  }
...
async function getRecipes() {
  ...
  {
    for(let count = 0; count < RECIPE_URLS.length; count++)
    {
      try 
      {
        const getRecipe = await fetch(RECIPE_URLS[count]);
        const recipeContent = await getRecipe.json();
        recipeList.push(recipeContent);
        if(count == RECIPE_URLS.length - 1)
        {
